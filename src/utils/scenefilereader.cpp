...
#define PARSE_ERROR(e)                                                                             \
    std::cout << ERROR_AT(e) << "could not parse <" << e.tagName().toStdString() << ">" << std::endl
#define UNSUPPORTED_ELEMENT(e)                                                                     \
    std::cout << ERROR_AT(e) << "unsupported element <" << e.tagName().toStdString() << ">"        \
              << std::endl;
 ...
 */
template <typename T>
bool parseTriple(const QDomElement& triple, T& a, T& b, T& c, const QString& str_a,
                 const QString& str_b, const QString& str_c) {
    if (!triple.hasAttribute(str_a) || !triple.hasAttribute(str_b) || !triple.hasAttribute(str_c))
        return false;
 ...
 */
template <typename T>
bool parseQuadruple(const QDomElement& quadruple, T& a, T& b, T& c, T& d, const QString& str_a,
                    const QString& str_b, const QString& str_c, const QString& str_d) {
...
/**
 * Helper function to parse a texture map tag. The texture map image should be relative to the root
 * directory of scenefile root. Example texture map tag: <texture file="/image/andyVanDam.jpg" u="1"
 * v="1"/>
 */
    ...
    while (!childNode.isNull()) {
        ...
        if (e.tagName() == "id") {
            ...
            if (e.attribute("v") == "directional")
                ...
                std::cout << ERROR_AT(e) << "unknown light type " << e.attribute("v").toStdString()
                          << std::endl;
                return false;
            }
            ...
            if (!parseTriple(e, light->function.x, light->function.y, light->function.z, "a", "b",
                             "c") &&
                !parseTriple(e, light->function.x, light->function.y, light->function.z, "x", "y",
                             "z") &&
                !parseTriple(e, light->function.x, light->function.y, light->function.z, "v1", "v2",
                             "v3")) {
                PARSE_ERROR(e);
                return false;
            }
            ...
            if (light->type == LightType::LIGHT_DIRECTIONAL) {
                std::cout << ERROR_AT(e) << "position is not applicable to directional lights"
                          << std::endl;
                return false;
            }
            ...
            if (light->type == LightType::LIGHT_POINT) {
                std::cout << ERROR_AT(e) << "direction is not applicable to point lights"
                          << std::endl;
                return false;
            }
...
bool ScenefileReader::parseCameraData(const QDomElement& cameradata) {
    ...
    while (!childNode.isNull()) {
        QDomElement e = childNode.toElement();
        if (e.tagName() == "pos") {
            if (!parseTriple(e, m_cameraData.pos.x, m_cameraData.pos.y, m_cameraData.pos.z, "x",
                             "y", "z")) {
                PARSE_ERROR(e);
                return false;
            }
            ...
            if (!parseTriple(e, m_cameraData.look.x, m_cameraData.look.y, m_cameraData.look.z, "x",
                             "y", "z")) {
                PARSE_ERROR(e);
                return false;
            }
            ...
            if (!parseTriple(e, m_cameraData.up.x, m_cameraData.up.y, m_cameraData.up.z, "x", "y",
                             "z")) {
                PARSE_ERROR(e);
                return false;
            }
...
bool ScenefileReader::parseTransBlock(const QDomElement& transblock, SceneNode* node) {
    ...
    while (!childNode.isNull()) {
        ...
        if (e.tagName() == "translate") {
            ...
            float angle;
            if (!parseQuadruple(e, t->rotate.x, t->rotate.y, t->rotate.z, angle, "x", "y", "z",
                                "angle")) {
                PARSE_ERROR(e);
                return false;
            }
            ...
            if (e.attribute("type") == "master") {
                ...
                if (!m_objects[masterName]) {
                    std::cout << ERROR_AT(e) << "invalid master object reference: " << masterName
                              << std::endl;
                    return false;
                }
                ...
                std::cout << ERROR_AT(e)
                          << "invalid object type: " << e.attribute("type").toStdString()
                          << std::endl;
                return false;
            }
        } else if (!e.isNull()) {
