...
struct SceneMaterial {
    ...
    SceneColor
        cReflective; // Used to weight contribution of reflected ray lighting (via multiplication)
...
struct SceneTransformation {
    ...
    glm::vec3 translate; // Only applicable when translating. Defines t_x, t_y, and t_z, the amounts
                         // to translate by, along each axis.
    glm::vec3 scale;  // Only applicable when scaling.     Defines s_x, s_y, and s_z, the amounts to
                      // scale by, along each axis.
    glm::vec3 rotate; // Only applicable when rotating.    Defines the axis of rotation; should be a
                      // unit vector.
    float angle; // Only applicable when rotating.    Defines the angle to rotate by in RADIANS,
                 // following the right-hand rule.
    glm::mat4
        matrix; // Only applicable when transforming by a custom matrix. This is that custom matrix.
};

// Struct which represents a node in the scene graph/tree, to be parsed by the student's
// `SceneParser`.
struct SceneNode {
